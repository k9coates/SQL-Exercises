...
The facilities: Tennis Court1, Tennis Court2, Massage Room1, Massage Room2, and 
Squash Court charge a fee to members.
...
The name of the last member to sign up is Darren Smith.
...
WHERE
	CASE 
		WHEN b.memid <> 0 THEN (f.membercost * b.slots) > 30
		WHEN b.memid = 0 THEN (f.guestcost * b.slots) > 30
		END
...
SELECT f.name, sub.member_name, sub.cost
...
WHERE f.name = sub.facility_name AND
               sub.cost > 30
...
SELECT f.name AS Facility_Name, SUM(b.slots) AS Used_Slots
FROM Bookings AS b
LEFT JOIN Facilities AS f
ON b.facid = f.facid
WHERE b.memid <> 0
